...
function showLookup(psLookupURL, psHtmlTagID_ImgLoading, paSearchValue, paTargetTag) {
    $("#DialogLookupWait").show();
    $("#DialogLookupContent").empty();
    var vsURL = BaseUrl + psLookupURL;
    var voDTO = { paSearchValue: paSearchValue, paTargetTag: paTargetTag };
...
function showLookup_AdditemsXclude(psLookupURL, psHtmlTagID_Form, psHtmlTagID_ImgLoading) {
    $("#DialogLookupWait_additems").show();
    $("#DialogLookupContent_additems").empty();
    var vsURL = BaseUrl + psLookupURL;
    var voDTO = $("#" + psHtmlTagID_Form).serialize();
    jQuery.ajaxSettings.traditional = true;
    $.ajax({ type: AJAXTypePOST(), url: vsURL, data: voDTO, traditional: true
    }).done(function (data) {
        $("#DialogLookupContent_additems").html(data);
        //$('#DialogLookup').dialog('open');
    });   //End $.ajax
}; //End function showLookup
function showLookup_Additems(psLookupURL, psHtmlTagID_ImgLoading, paExistinglist, paSelectedlist) {
    $("#DialogLookupWait_additems").show();
    $("#DialogLookupContent_additems").empty();
    var vsURL = BaseUrl + psLookupURL;
    var voDTO = { paExistinglist: paExistinglist, paSelectedlist: paSelectedlist };
    jQuery.ajaxSettings.traditional = true;
    $.ajax({ type: AJAXTypeGET(), url: vsURL, data: voDTO, traditional: true
    }).done(function (data) {
        $("#DialogLookupContent_additems").html(data);
...
function showLookup_Result(psLookupURL, psHtmlTagID_ImgLoading, paExistinglist, paSelectedlist, paSelectnewlist) {
    $("#DialogLookupWait_additems").show();
    $("#DialogLookupContent_additems").empty();
    var vsURL = BaseUrl + psLookupURL;
    var voDTO = { paExistinglist: paExistinglist, paSelectedlist: paSelectedlist, paSelectnewlist: paSelectnewlist };
    jQuery.ajaxSettings.traditional = true;
    $.ajax({ type: AJAXTypeGET(), url: vsURL, data: voDTO, traditional: true
    }).done(function (data) {
        $("#RSLTDATA_TAG").html(data);
        //$("#DialogLookupContent_additems").html(data);
        //$('#DialogLookup').dialog('open');
    });   //End $.ajax
}; //End function showLookup
...
function inputInteger_BACKUP20160929() {
    $(".data-mask-integer").keydown(function (e) {
...
} //End function inputInteger_BACKUP20160929()
//***** refactored *****
function inputInteger() {
    $('.data-mask-integer').on('input', function (event) {
        this.value = this.value.replace(/[^0-9]/g, '');
    });
} //End function inputInteger()
//***** refactored *****
function inputIntegerPct() {
    $(".data-mask-integer-percentage").keydown(function (event) {
    ...
    $(".data-mask-integer-percentage").focusout(function (event) {
        if ($(this).val() > 100) {
            $(this).val('100');
        }
    });
} //End function inputIntegerPercentage()
...
} //End function inputDecimal(psID)
function inputDecimalPct() {
...
} //End function inputDecimalPercentage()
function showXOver(psLookupURL, psHtmlTagID_ImgLoading) {
    //$("#DialogLookupWait").show();
    $("#DialogLookupContent").empty();
    var vsURL = BaseUrl + psLookupURL;
    var voDTO = { XOver: "x" };
...
function Select2ajax(psSelectID, psURL) {
    $.getJSON(psURL, function (data) {
        $(psSelectID).select2({
            data: data
        });
    });
}; //End function select2ajax
function Select2ajaxfilter(psSelectID, psURL, psFilterID1, psFilterID2, psFilterID3) {

    var sFilter1 = "";
    var sFilter2 = "";
    var sFilter3 = "";
    Select2ajax(psSelectID, psURL);

    if (psFilterID1 != "") {
        $(psFilterID1).on("change", function (e) {
            if (psFilterID1 != "") sFilter1 = $(psFilterID1).val();
            if (psFilterID2 != "") sFilter2 = $(psFilterID2).val();
            if (psFilterID3 != "") sFilter3 = $(psFilterID3).val();

            $.getJSON(psURL,
            { psFilter1: sFilter1, psFilter2: sFilter2, psFilter3: sFilter3 },
            function (data) {
                $(psSelectID).empty().select2({
                    data: data
                });
            });
        });    //End $(psFilterID1).on("change", function (e) {
    } //End if (psFilterID1 != "")
    
    if (psFilterID2 != "") { } //End if (psFilterID2 != "")
    
    if (psFilterID3 != "") { } //End if (psFilterID3 != "")
}; //End function Select2ajaxfilter
