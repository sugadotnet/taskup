...
namespace APPBASE.Models
{
    public class UnitCRUD
    {
        ...
        //Constructor 1
        public UnitCRUD() { this.db = new DBMAINContext(); } //End public UnitCRUD()
        //Constructor 2
        public UnitCRUD(DBMAINContext poDB)
        { this.db = poDB; } //End public UnitCRUD()

        public void Create(UnitVM poViewModel)
        {
            try
            {
                this.oModel = new Unit();
                //Map Form Data
                ...
                //Process CRUD
                this.db.Units.Add(this.oModel);
                //this.db.SaveChanges();
                //this.ID = this.oModel.ID;
            } //End try
            catch (Exception e) { isERR = true; this.ERRMSG = "CRUD - Create: " + e.Message; } //End catch
        } //End public void Create
        public void Update(UnitVM poViewModel)
        {
            try
            {
                this.oModel = this.db.Units.AsNoTracking().SingleOrDefault(fld => fld.ID == poViewModel.ID);
                //Map Form Data
                this.oModel.InjectFrom(poViewModel);
                //Set Field Header
        ...
        public void Delete(int? id)
        {
            try
            {
                this.oModel = this.db.Units.Find(id);
                this.db.Units.Remove(this.oModel);
                //this.db.SaveChanges();
                //this.ID = this.oModel.ID;
            } //End try
